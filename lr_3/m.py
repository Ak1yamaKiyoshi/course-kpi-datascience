...
statistics_table = pd.DataFrame({
    ...
    'Після згладжування Калманом з відновленням (reject)': [np.mean(smoothed_restored_reject_measurments), np.std(smoothed_restored_reject_measurments)],
})
...
plt.subplot(3, 1, 1)
plt.plot(smoothed_restored_reject_measurments, label='Результат згладжування Калмана відновлених методом відкидання', linestyle='--', color='purple')
plt.legend()
plt.title('Результат згладжування Калмана відновлених методом відкидання')
...
plt.subplot(3, 1, 3)
plt.plot(smoothed_restored_reject_measurments, label='Результат згладжування Калмана зашумлених даних без аномалій', linestyle='--', color='purple')
plt.legend()
...
axs[2].hist(smoothed_restored_reject_measurments, alpha=0.5, color='green', label='Злагоджене з аномаліями ( метод відкидання )')
axs[2].set_title('Злагоджене з аномаліями ( метод відкидання ) ')
