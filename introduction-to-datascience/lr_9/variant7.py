...
# Invert image colors and add random noise
def distort_image(image):
    # Invert colors
    inverted_image = cv2.bitwise_not(image)
   
    # Add random noise
    noise = np.random.normal(0, 1, inverted_image.shape).astype(np.uint8)
    return cv2.add(inverted_image, noise)
