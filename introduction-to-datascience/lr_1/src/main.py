...
def constantionous_law(size=1000, show=False):
    # Створення моделі з постійним законом зміни (лінійний зріст)
    def constant_change_model(size):
        time = np.arange(size)  # Часова відмітка
        rate_of_change = 0.01  # Постійний коефіцієнт зміни
        return time * rate_of_change

    # Розмір вибірки та створення моделі
    data = constant_change_model(size)
    if show: 
        # Візуалізація результатів
